...
class UITestExecutor:
    ...
    async def execute(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 使用 sync_to_async 异步获取步骤列表
            # 注意：这里直接获取所有步骤对象，避免后续懒加载
            steps = await sync_to_async(
                lambda: list(self.test_case.ui_steps.all().order_by('step_number'))
            )()

            if not steps:  # 检查列表是否为空
                logger.warning(f"测试步骤为空: {self.test_case.name}")
                return self._create_result('failed', error_message='没有步骤')
    ...
    async def _setup_browser(self):
        ...
        try:
            ...
            logger.info(f"浏览器设置成功: {browser_type}")
    ...
    async def _handle_click(self, step):
        ...
        await locator.click()
        logger.info(f"元素点击成功: {step.element_selector}")
    ...
    async def _handle_double_click(self, step):
        ...
        logger.info(f"双击成功: {step.element_selector}")
    ...
    async def _handle_right_click(self, step):
        ...
        logger.info(f"右击成功: {step.element_selector}")
    ...
    async def _handle_fill(self, step):
        ...
        logger.info(f"填写文本成功: {step.element_selector} = {value}")
    ...
    async def _handle_select(self, step):
        ...
        logger.info(f"选择成功: {step.element_selector} = {value}")
    ...
    async def _handle_hover(self, step):
        ...
        await locator.hover()
        logger.info(f"元素悬停成功: {step.element_selector}")
    ...
    async def _handle_wait(self, step):
        ...
        logger.info(f"等待成功: {step.element_selector}")
    ...
    async def _handle_assert_visible(self, step):
        ...
        if not is_visible:
            raise AssertionError(f"元素不可见: {step.element_selector}")

        logger.info(f"断言成功: 元素可见: {step.element_selector}")
        ...
        try:
            ...
            if expected_text not in actual_text:
                raise AssertionError(
                    f"断言失败: expected '{expected_text}' in '{actual_text}'"
                )

            logger.info(f"断言成功: text contains '{expected_text}'")

        except ElementNotFoundError:
            raise AssertionError(f"元素不存在: {step.element_selector}")
    ...
    async def _handle_assert_url(self, step):
        ...
        if expected_url not in current_url:
            raise AssertionError(
                f"URL 断言失败: expected '{expected_url}' in '{current_url}'"
            )

        logger.info(f"断言成功: URL contains '{expected_url}'")
    ...
    async def _handle_scroll(self, step):
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"非法的坐标值: {value}")

        await self.driver.scroll_to(x, y)
        logger.info(f"滚动到坐标: ({x}, {y})")
